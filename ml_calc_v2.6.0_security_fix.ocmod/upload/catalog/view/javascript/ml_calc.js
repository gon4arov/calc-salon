...
window.MLCalc.init = function(wrapper) {
    ...
    function updateProductPriceField(value) {
    ...
    function setupOptionPriceSync() {
        ...
        function recomputePrice() {
            ...
            updateProductPriceField(convertedSpecialPrice);
            updateSummaryPriceDisplay(convertedSpecialPrice, convertedRegularPrice);
            calculatePayback();
        }

        $optionScope.on('change.mlCalc' + calcId + ' input.mlCalc' + calcId, 'input[name^="option["], select[name^="option["], textarea[name^="option["]', function() {
            recomputePrice();
        });
    ...
    // Автоматичний перерахунок при зміні слайдерів
    find('.ml-calc-slider').on('change', function() {
        calculatePayback();
    });
    ...
    function updateResultTooltips(json, clients, cost, days, rent, utilities, percent, price) {
        ...
        var paybackMonths = Math.ceil(paybackDays / 30);
        ...
        var paybackFormula =
            ...
            labels.payback + ' = ' + price.toFixed(0) + ' ÷ ' + monthlyProfit.toFixed(0) + ' = ' + paybackDays.toFixed(1) + ' ' + labels.days + ' (' + paybackMonths + ' ' + labels.monthsShort + ')';
        ...
        if (json.payback_days_regular_raw && json.price_regular_raw) {
            ...
            var paybackMonthsRegular = Math.ceil(paybackDaysRegular / 30);
            ...
            var paybackRegularFormula =
                ...
                labels.payback + ' = ' + priceRegular.toFixed(0) + ' ÷ ' + monthlyProfit.toFixed(0) + ' = ' + paybackDaysRegular.toFixed(1) + ' ' + labels.days + ' (' + paybackMonthsRegular + ' ' + labels.monthsShort + ')';
